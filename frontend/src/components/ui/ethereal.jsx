...
const ScrollHero = ({
  sections = [
    { id: 'hero', headline: 'Smart Trolly', subheadline: 'Beyond Checkout', body: 'AI-powered autonomous retail with real-time neural vision' },
    { id: 'about', headline: 'YOLOv8', subheadline: 'Neural Detection', body: 'Sub-second optical item detection with zero GPU dependency' },
    { id: 'services', headline: 'One Tap', subheadline: 'Checkout Flow', body: 'GST-compliant billing, Razorpay payments & thermal receipts' },
    { id: 'contact', headline: 'Start Now', subheadline: 'Scan & Pay', body: 'Launch the live scanner and experience frictionless checkout' }
  ],
  colorPalette = {
    primary: '#10b981',
    secondary: '#059669',
  ...
  return (
    <div ref={containerRef} className="scroll-hero">
      ...
      <nav className="ethereal-nav">
        <div className="ethereal-nav-inner">
          <div className="ethereal-nav-logo">{logo}</div>
          <div className="ethereal-nav-menu">
            {menuItems.map((item, i) => (
              <a
                key={i}
                href={`#${item.toLowerCase()}`}
                className={`ethereal-nav-item ${activeSection === i ? 'active' : ''}`}
              >
                {item}
              </a>
            ))}
      ...
      <div className={`ethereal-loading-overlay ${isLoaded ? 'loaded' : ''}`}>
        <div className="ethereal-loading-text">
          <span>Loading</span>
          <span className="ethereal-dots">...</span>
        </div>
      </div>
    </div>
  );
